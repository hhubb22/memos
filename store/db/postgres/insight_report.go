...
func (d *DB) CreateInsightReport(ctx context.Context, create *store.InsightReport) (*store.InsightReport, error) {
	...
	stmt := "INSERT INTO insight_report (" + strings.Join(fields, ", ") + ") VALUES (" + placeholders(len(args)) + ") RETURNING id, created_ts, updated_ts"
...
func (d *DB) DeleteInsightReport(ctx context.Context, delete *store.DeleteInsightReport) error {
	result, err := d.db.ExecContext(ctx, "DELETE FROM insight_report WHERE id = $1", delete.ID)
	if err != nil {
		return err
	}
